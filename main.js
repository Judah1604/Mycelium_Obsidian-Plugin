...
async function getRelatedNotes(keywords, subjectFile, app2) {
...
async function getAnalysis(file, fileName, apiKey2, app2) {
...
async function compareNote(app2, apiKey2) {
  ...
  if (relatedNotes.length === 0) {
    ...
    await writeReport(analysisText, activeFile.basename, "analysis", app2);
  ...
  const selectedNotes = await selectRelatedNotes(relatedNotes, app2);
  ...
  await writeReport(fileName, comparisonResponse, reportType, app2);
  new import_obsidian2.Notice("\u2713 Reports Saved");
}
...
var ActionSelectModal = class extends import_obsidian3.Modal {
  constructor(app2, resolve) {
    super(app2);
    this.resolve = resolve;
  }
  onOpen() {
    ...
    new import_obsidian3.Setting(contentEl).setName("Analyze this note").setDesc("Identify assumptions, weak points, and questions in this note.").addButton(
      (button) => button.setButtonText("Analyze").onClick(() => {
        this.resolve("analyze");
        this.close();
      })
    );
    new import_obsidian3.Setting(contentEl).setName("Compare this note").setDesc("Compare this note against related notes in your vault.").addButton(
      (button) => button.setButtonText("Compare").setCta().onClick(() => {
        this.resolve("compare");
        this.close();
      })
    );
    new import_obsidian3.Setting(contentEl).setName("Build Analysis Cache").setDesc("Build analysis cache for all notes.").addButton(
      (button) => button.setButtonText("Run").setCta().onClick(() => {
...
// src/helpers/selectAction.js
function selectAction(app2) {
  return new Promise((resolve) => {
    new ActionSelectModal(app2, resolve).open();
  });
}
...
var Mycelium = class extends import_obsidian4.Plugin {
  async onload() {
    await this.loadSettings();
    new import_obsidian4.Notice("Mycelium is alive");
...
var MyceliumSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian4.Setting(containerEl).setName("API Key").setDesc("Your LLM API key").addText(
