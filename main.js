...
async function writeReport2(subjectFile2, report, reportType2, app2) {
  ...
  if (reportType2 == "analysis") {
    if (!await app2.vault.adapter.exists("Mycelium/analyses/")) {
      await app2.vault.createFolder("Mycelium/analyses/");
    }
    const mdPath2 = `Mycelium/analyses/${safeName}/${safeName}.md`;
    const jsonPath2 = `Mycelium/analyses/${safeName}/${safeName}.json`;
  } else if (reportType2 == "comparison") {
    if (!await app2.vault.adapter.exists("Mycelium/comparison/")) {
      await app2.vault.createFolder("Mycelium/comparison/");
    }
    const mdPath2 = `Mycelium/comparisons/${safeName}/${safeName}.md`;
    const jsonPath2 = `Mycelium/comparisons/${safeName}/${safeName}.json`;
  }
  await app2.vault.adapter.write(mdPath, report);
  await app2.vault.adapter.write(
    jsonPath,
...
async function compareNote(app2, apiKey2) {
  ...
  const relatedNotes = await getRelatedNotes(keywords, subjectFile2, app2);
  if (relatedNotes.length === 0) {
    new import_obsidian2.Notice("No related notes found.");
    await writeResults(analysis, null, subjectFile2, app2);
  ...
  await writeReport2(comparisonResponse2, subjectFile2, reportType2, app2);
  new import_obsidian2.Notice("\u2713 Reports Saved");
}
...
async function analyzeNote2(app2, apiKey2, file) {
  ...
  console.log(file);
  const analysis = getAnalysis(file, fileName, apiKey2, app2);
  if (!analysis) {
    new Notice("Analysis failed.");
    return;
  }
  new Notice(`\u2713 ${fileName} analysis complete`);
  await writeReport2(fileName, analysis, reportType2, app2);
...
async function buildAnalysisCache(app2, apiKey2) {
  ...
  for (const file of files) {
    ...
    if (exists) continue;
    const { analysis, subjectFile: subjectFile2 } = await analyzeNote2(app2, apiKey2, file);
...
var Mycelium = class extends import_obsidian3.Plugin {
  async onload() {
    ...
    this.addRibbonIcon("mycelium-icon", "Mycelium", async () => {
      await analyzeNote2(this.app, this.settings.apiKey);
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        await analyzeNote2(this.app, this.settings.apiKey, activeFile);
      }
    });
    this.addCommand({
